...
// Phase 2 storage migration: today's/each day's ORDERS array (previously localStorage
// key kopi_orders_v1, client-side only) now lives in KV under one key per calendar date,
...
// Approved responsive supplier and material-management surfaces.  They are applied to the
// production page at response time so the current parsing and Cloudflare order API remain
// the single source of truth.
const RESPONSIVE_UI = String.raw`<style>
.supplier-page,.material-page,.dispatch-page{max-width:1180px;margin:auto}.supplier-headline,.dispatch-head,.material-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:16px}.supplier-headline h2,.dispatch-head h2,.material-head h2{margin:0;color:#1c1b1a;font-size:22px}.supplier-headline p,.dispatch-head p,.material-head p{margin:4px 0;color:#6b6862;font-size:13px}.supplier-workspace{display:grid;grid-template-columns:360px minmax(0,1fr);gap:20px}.supplier-inbox,.supplier-board-wrap,.material-page{background:#fff;border:1px solid #e5e3de;border-radius:14px;padding:16px}.supplier-label{display:flex;justify-content:space-between;color:#6b6862;font-size:12px;font-weight:700;text-transform:uppercase;margin-bottom:12px}.supplier-stack{display:grid;gap:10px}.supplier-card{background:#f7f6f3;border:1px solid #e5e3de;border-radius:12px;padding:12px}.supplier-card-top{display:flex;justify-content:space-between;gap:8px}.supplier-card-name{font-weight:700}.supplier-card-meta{font-size:12px;color:#6b6862}.supplier-card-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:8px}.supplier-card-actions button{background:none;color:#2b4c7e;border:0;padding:0;text-decoration:underline;font-size:12px}.supplier-card-actions button[data-ui="remove"]{background:#fbe9e7;border:1px solid #e7b7b1;border-radius:7px;color:#a12d26;font-weight:700;padding:4px 8px;text-decoration:none;white-space:nowrap}.supplier-card select{width:100%;margin-top:8px;padding:8px;border:1px solid #e5e3de;border-radius:8px}.supplier-board{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.supplier-column{border:1px solid #e5e3de;border-radius:12px;padding:12px;min-height:130px}.supplier-column-head{display:flex;justify-content:space-between;font-weight:700;font-size:13px}.supplier-column-count,.material-badge{border-radius:999px;background:#edebe6;color:#6b6862;padding:2px 8px;font-size:11px}.supplier-empty{color:#9b968c;font-size:12px;margin-top:10px}.material-toolbar{display:flex;gap:10px;flex-wrap:wrap;margin:16px 0}.material-toolbar input{flex:1 1 220px;padding:10px;border:1px solid #e5e3de;border-radius:8px;background:#f7f6f3}.material-toolbar button{background:#2b4c7e}.material-page .card{border:0;padding:0;margin:0}.material-page .info-banner{display:none}.material-page .menu-item-card{box-shadow:none;border:0;border-top:1px solid #e5e3de;border-radius:0;margin:0;padding:13px 0}.material-page .category-group-head{margin-top:16px}.dispatch-head{border-bottom:1px solid #e5e3de;padding-bottom:14px}@media(max-width:700px){.supplier-headline,.dispatch-head,.material-head{flex-direction:column}.supplier-workspace{grid-template-columns:1fr;gap:12px}.supplier-board{grid-template-columns:1fr}.supplier-inbox,.supplier-board-wrap,.material-page{padding:14px}.supplier-headline h2,.dispatch-head h2,.material-head h2{font-size:19px}.material-toolbar{flex-direction:column}.material-toolbar button{width:100%}}
</style><script>
(()=>{
  let picker=null;
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const rows=()=>{const out=[];ORDERS.filter(o=>o.status!=="dispatched").forEach(o=>(o.items||[]).forEach(i=>out.push({o,i})));return out};
  const suppliers=()=>allSuppliers().filter(Boolean);
  function card(r){
    const k=r.o.id+'|'+r.i.id,open=picker===k;
    return '<article class="supplier-card"><div class="supplier-card-top"><div><div class="supplier-card-name">'+esc(r.i.label)+'</div><div class="supplier-card-meta">'+esc(r.o.department||r.o.orderer||'ออเดอร์')+'</div></div><div class="supplier-card-meta">'+esc(r.i.qty||'?')+' '+esc(r.i.unit||'')+'</div></div><div class="supplier-card-actions"><button data-ui="move" data-o="'+esc(r.o.id)+'" data-i="'+esc(r.i.id)+'">ย้ายซัพ</button><button data-ui="qty" data-o="'+esc(r.o.id)+'" data-i="'+esc(r.i.id)+'">แก้จำนวน</button><button data-ui="label" data-o="'+esc(r.o.id)+'" data-i="'+esc(r.i.id)+'">แก้ชื่อ</button><button data-ui="remove" data-o="'+esc(r.o.id)+'" data-i="'+esc(r.i.id)+'">ยกเลิกรายการ</button></div>'+(open?'<select data-ui-supplier data-o="'+esc(r.o.id)+'" data-i="'+esc(r.i.id)+'"><option value="">เลือกซัพพลายเออร์</option>'+suppliers().map(s=>'<option value="'+esc(s)+'"'+(s===r.i.supplier?' selected':'')+'>'+esc(s)+'</option>').join('')+'</select>':'')+'</article>';
  }
  function renderSupplierUI(){
    const all=rows(),un=all.filter(r=>!r.i.supplier),assigned=all.filter(r=>r.i.supplier),list=document.querySelector('#ordersList'),board=document.querySelector('#supplierBoard');
    if(!list||!board)return;
    list.innerHTML=un.length?un.map(card).join(''):'<div class="supplier-empty">ไม่มีรายการที่ต้องเลือกซัพพลายเออร์</div>';
    const groups={};assigned.forEach(r=>(groups[r.i.supplier]??=[]).push(r));
    const names=[...new Set([...suppliers(),...Object.keys(groups)])];
    board.innerHTML=names.map(n=>'<section class="supplier-column"><div class="supplier-column-head"><span>'+esc(n)+'</span><span class="supplier-column-count">'+(groups[n]||[]).length+' รายการ</span></div>'+((groups[n]||[]).map(card).join('')||'<div class="supplier-empty">ยังไม่มีรายการ</div>')+'</section>').join('');
    const badge=document.querySelector('#pendingCountBadge');if(badge){badge.textContent=un.length;badge.style.display='inline-block'}
    const title=document.querySelector('#supplierSub');if(title)title.textContent=un.length?un.length+' รายการยังไม่ระบุซัพพลายเออร์':'จัดกลุ่มครบทุกรายการแล้ว';
  }
  const legacyDispatch=renderDispatch;
  renderOrders=renderSupplierUI;
  renderDispatch=function(){legacyDispatch()};
  document.addEventListener('click',e=>{
    const b=e.target.closest('[data-ui]');if(!b)return;
    const o=b.dataset.o,i=b.dataset.i;
    if(b.dataset.ui==='move'){picker=picker===o+'|'+i?null:o+'|'+i;renderOrders()}
    if(b.dataset.ui==='qty'){
      const r=rows().find(x=>x.o.id===o&&x.i.id===i),v=r&&prompt('จำนวนใหม่สำหรับ '+r.i.label,r.i.qty||'');
      if(v!==null&&r){editItemQty(o,i,v,r.i.unit);renderOrders();renderDispatch()}
    }
    if(b.dataset.ui==='label'){
      const r=rows().find(x=>x.o.id===o&&x.i.id===i),v=r&&prompt('ชื่อวัตถุดิบ',r.i.label);
      if(v&&r){editItemLabel(o,i,v.trim());saveOrders();renderOrders();renderDispatch()}
    }
    if(b.dataset.ui==='remove'){
      const order=ORDERS.find(order=>order.id===o),item=order&&(order.items||[]).find(item=>item.id===i);
      if(item&&window.confirm('ยกเลิกรายการ "'+item.label+'" ใช่ไหม?')){
        order.items=order.items.filter(item=>item.id!==i);
        if(!order.items.length)ORDERS=ORDERS.filter(order=>order.id!==o);
        saveOrders();renderOrders();renderDispatch();showToast('ยกเลิกรายการแล้ว');
      }
    }
  });
  document.addEventListener('change',e=>{if(!e.target.matches('[data-ui-supplier]')||!e.target.value)return;picker=null;confirmItemSupplier(e.target.dataset.o,e.target.dataset.i,e.target.value)});
  renderOrders();
})();
</script>`;
...
// Redesign the ingredient/match-word workspace around the live dictionary data and the
// existing edit controls. The grouping, filtering, and test result are presentation-only;
// every add/edit/delete/alias action still uses the established persistence functions.
const MATCH_WORDS_UI = String.raw`<style>
.match-words-page{max-width:880px;margin:0 auto}.match-search{position:sticky;top:0;z-index:20;background:#f7f6f3;padding:10px 0}.match-search-box{position:relative}.match-search-box input{width:100%;border:1px solid #e7e3d8;border-radius:12px;padding:13px 14px 13px 42px;background:#fff;font-size:15px;box-shadow:0 1px 3px rgba(20,20,18,.06)}.match-search-box input:focus{outline:2px solid #14b8a6;border-color:transparent}.match-search-icon{position:absolute;left:14px;top:50%;transform:translateY(-50%);pointer-events:none}.match-hint{margin:6px 2px 0;color:#9a9587;font-size:12px}.match-test{margin-top:10px;border:1px solid;border-radius:12px;padding:11px 13px;font-size:13px}.match-test strong{display:block;margin-bottom:2px}.match-test.ok{background:#dcf3e8;border-color:#b7e4cc;color:#15603b}.match-test.warn{background:#fdecd8;border-color:#f3c99d;color:#8a4a12}.match-test.none{background:#fbeae8;border-color:#efc3be;color:#7e2b25}.match-stats{display:flex;gap:8px;flex-wrap:wrap;margin:14px 0 10px}.match-stat{background:#fff;border:1px solid #e7e3d8;border-radius:12px;padding:8px 12px;flex:1;min-width:120px}.match-stat b{display:block;font-size:18px}.match-stat span{color:#9a9587;font-size:12px}.match-stat.alert b{color:#c2691a}.match-toolbar{display:flex;justify-content:space-between;align-items:center;gap:8px;margin:14px 0 10px;flex-wrap:wrap}.match-toolbar-title{font-weight:700;font-size:14px}.match-segment{display:inline-flex;background:#f3f1ea;border-radius:10px;padding:3px;gap:2px}.match-segment button{border:0;background:transparent;padding:6px 12px;border-radius:8px;color:#767267;font-size:12.5px;font-weight:600}.match-segment button.on{background:#fff;color:#22201c;box-shadow:0 1px 2px rgba(0,0,0,.08)}.match-filters{display:flex;gap:8px;overflow-x:auto;padding-bottom:4px;margin-bottom:10px}.match-filter{flex:0 0 auto;border:1px solid #e7e3d8;border-radius:999px;background:#fff;color:#767267;padding:6px 14px;font-size:13px;font-weight:600}.match-filter.on{background:#0d7377;border-color:#0d7377;color:#fff}.match-group{background:#fff;border:1px solid #e7e3d8;border-radius:14px;margin-bottom:12px;overflow:hidden;box-shadow:0 1px 3px rgba(20,20,18,.06)}.match-group-head{display:flex;align-items:center;gap:10px;padding:12px 14px;cursor:pointer}.match-group-avatar{width:36px;height:36px;border-radius:10px;background:#eaf7f5;color:#0d7377;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:13px;flex:none}.match-group-main{flex:1;min-width:0}.match-group-name{font-size:14.5px;font-weight:700}.match-group-meta{font-size:12px;color:#9a9587}.match-group-chevron{color:#9a9587}.match-group.closed .match-group-body{display:none}.match-group.closed .match-group-chevron{transform:rotate(-90deg)}.match-words-page .menu-item-card{border:0;border-top:1px solid #f0ede3;border-radius:0;box-shadow:none;margin:0;padding:13px 14px}.match-words-page .menu-item-card:first-child{border-top:0}.match-words-page .menu-item-top{align-items:center}.match-words-page .menu-item-name{font-size:14px}.match-words-page .alias-pills{margin:8px 0}.match-words-page .inline-form{border-top:1px dashed #e7e3d8}.match-empty{text-align:center;padding:36px 16px;color:#9a9587}.match-words-page .material-toolbar{margin:0}.match-words-page .material-page .card{background:transparent}.match-add-form{display:none;background:#fff;border:1px solid #e7e3d8;border-radius:14px;padding:16px;margin-top:12px;box-shadow:0 1px 3px rgba(20,20,18,.06)}.match-add-form .field-label{display:block;font-size:12px;font-weight:600;color:#767267;margin:0 0 4px}.match-add-form .grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.match-add-form input,.match-add-form select{width:100%;padding:10px 12px;border:1px solid #d8d3c4;border-radius:10px;background:#fff}.match-add-form .row{margin-top:14px}@media(max-width:600px){.match-add-form .grid{grid-template-columns:1fr}.match-stat{min-width:0;padding:8px 10px}.match-stat b{font-size:16px}}
</style><script>
(()=>{
  let groupBy="supplier",activeFilter="__all";
  const closedGroups=new Set();
  const search=document.getElementById("dictSearch"),result=document.getElementById("matchTestResult"),filters=document.getElementById("matchFilters"),groups=document.getElementById("menuGroups");
  if(!search||!result||!filters||!groups)return;
  const addForm=document.getElementById("addItemForm"),addButton=document.getElementById("addItemToggleBtn"),addCancel=document.getElementById("addItemCancelBtn"),addSave=document.getElementById("addDictBtn");
  addForm.style.display="none";
  addCancel.addEventListener("click",()=>{addForm.style.display="none";addButton.style.display="block"});
  addSave.addEventListener("click",()=>{addForm.style.display="none";addButton.style.display="block"});
  const aliasesFor=name=>ALIAS_TABLE.map((alias,index)=>Object.assign({},alias,{index})).filter(alias=>normalize(alias.canonical)===normalize(name));
  const searchable=(item,term)=>!term||(normalize(item.name)+normalize(item.supplier)+normalize(item.category||"")+aliasesFor(item.name).map(alias=>normalize(alias.typed)).join(" ")).includes(term);
  function updateTest(){
    const raw=search.value.trim();
    if(!raw){result.innerHTML='<p class="match-hint">พิมพ์ชื่อวัตถุดิบหรือคำที่พนักงานส่งมา ระบบจะบอกว่าจะแมทไปที่ใด</p>';return}
    const matches=findSupplierMatches(raw);
    if(!matches.length){result.innerHTML='<div class="match-test none"><strong>❓ ยังไม่รู้จักคำนี้</strong>ถ้ามาในออเดอร์จริง ระบบจะให้เลือกซัพพลายเออร์เอง</div>';return}
    if(matches.length>1){result.innerHTML='<div class="match-test warn"><strong>⚠️ ตรงกับหลายซัพพลายเออร์</strong>'+matches.map(match=>escapeAttr(match.supplier)+' ('+escapeAttr(match.canonicalName)+')').join(', ')+' — ระบบจะให้เลือกเอง</div>';return}
    const match=matches[0];
    if(match.missingSupplier){result.innerHTML='<div class="match-test warn"><strong>⚠️ แมทได้ แต่ยังไม่มีซัพพลายเออร์</strong>แมทเป็น “'+escapeAttr(match.canonicalName)+'” แต่ยังไม่ได้ผูกซัพ</div>';return}
    if(match.viaContainment){result.innerHTML='<div class="match-test warn"><strong>🤔 ระบบเดาว่าเป็น '+escapeAttr(match.canonicalName)+'</strong>ซัพ '+escapeAttr(match.supplier)+' — ออเดอร์จริงจะให้ยืนยันก่อน</div>';return}
    const how=match.viaAlias?'ผ่านคำแมท':match.viaFirstWord?'จากคำแรก':'ตรงเป๊ะ';
    result.innerHTML='<div class="match-test ok"><strong>✅ ส่งให้ '+escapeAttr(match.supplier)+' ทันที</strong>'+how+' → “'+escapeAttr(match.canonicalName)+'”</div>';
  }
  function populateAddFields(){
    const supplier=document.getElementById("newItemSupplier"),category=document.getElementById("newItemCategory");
    if(supplier)supplier.innerHTML=allSuppliers().map(name=>'<option value="'+escapeAttr(name)+'">'+escapeAttr(name)+'</option>').join('')+'<option value="__new__">+ ซัพพลายเออร์ใหม่</option>';
    if(category)category.innerHTML=allCategories().map(name=>'<option value="'+escapeAttr(name)+'">'+escapeAttr(name)+'</option>').join('')+'<option value="__newcat__">+ หมวดใหม่</option>';
  }
  function addFilter(value,label){const button=document.createElement("button");button.className="match-filter"+(activeFilter===value?" on":"");button.dataset.filter=value;button.textContent=label;filters.appendChild(button)}
  function renderGroups(){
    const term=normalize(search.value),keys=groupBy==="supplier"?allSuppliers():allCategories().filter(category=>DICT.some(item=>(item.category||"อื่นๆ")===category));
    if(activeFilter!=="__all"&&activeFilter!=="__noprice"&&!keys.includes(activeFilter))activeFilter="__all";
    filters.innerHTML="";addFilter("__all","ทั้งหมด");keys.forEach(key=>addFilter(key,key));addFilter("__noprice","⚠️ ยังไม่ใส่ราคา");
    groups.innerHTML="";let shown=false;
    keys.forEach(key=>{
      if(activeFilter!=="__all"&&activeFilter!=="__noprice"&&activeFilter!==key)return;
      const indexes=DICT.map((item,index)=>({item,index})).filter(entry=>(groupBy==="supplier"?entry.item.supplier:(entry.item.category||"อื่นๆ"))===key).filter(entry=>activeFilter!=="__noprice"||!entry.item.price).filter(entry=>searchable(entry.item,term)).sort((a,b)=>a.item.name.localeCompare(b.item.name,"th"));
      if(!indexes.length)return;shown=true;
      const group=document.createElement("section");group.className="match-group"+(term||activeFilter!=="__all"||!closedGroups.has(key)?"":" closed");
      const head=document.createElement("div");head.className="match-group-head";head.innerHTML='<div class="match-group-avatar"></div><div class="match-group-main"><div class="match-group-name"></div><div class="match-group-meta"></div></div><span class="match-group-chevron">▾</span>';
      head.querySelector(".match-group-avatar").textContent=key.slice(0,2);head.querySelector(".match-group-name").textContent=(groupBy==="supplier"?"ซัพ ":"")+key;head.querySelector(".match-group-meta").textContent=indexes.length+" รายการ · "+indexes.reduce((sum,entry)=>sum+aliasesFor(entry.item.name).length,0)+" คำแมท";
      head.addEventListener("click",()=>{group.classList.toggle("closed");group.classList.contains("closed")?closedGroups.add(key):closedGroups.delete(key)});
      const body=document.createElement("div");body.className="match-group-body";indexes.forEach(entry=>body.appendChild(buildMenuItemCard(entry.index)));group.appendChild(head);group.appendChild(body);groups.appendChild(group);
    });
    if(!shown)groups.innerHTML='<div class="match-empty">ไม่พบวัตถุดิบที่ตรงกับคำค้นหา</div>';
  }
  renderMenuGroups=function(){
    document.getElementById("dictCount").textContent=DICT.length;
    document.getElementById("matchStatItems").textContent=DICT.length;
    document.getElementById("matchStatAlias").textContent=ALIAS_TABLE.length;
    document.getElementById("matchStatNoPrice").textContent=DICT.filter(item=>!item.price).length;
    populateAddFields();updateTest();renderGroups();
  };
  search.addEventListener("input",renderMenuGroups);
  filters.addEventListener("click",event=>{const button=event.target.closest("button[data-filter]");if(!button)return;activeFilter=button.dataset.filter;renderMenuGroups()});
  document.getElementById("matchGroupMode").addEventListener("click",event=>{const button=event.target.closest("button[data-mode]");if(!button)return;groupBy=button.dataset.mode;activeFilter="__all";closedGroups.clear();document.querySelectorAll("#matchGroupMode button").forEach(item=>item.classList.toggle("on",item===button));renderMenuGroups()});
  renderMenuGroups();
})();
</script>`;

// Bring the live ingredient workspace in line with the supplied prototype.  This only
// replaces the presentation and interaction shell; it continues to use the same live
// dictionary, alias, and persistence helpers as the rest of the dashboard.
const MATCH_WORDS_PROTOTYPE_UI = String.raw`<style>
.material-page.match-words-page{max-width:880px;margin:0 auto;padding:16px 16px 80px;background:transparent;border:0;border-radius:0;box-shadow:none}.match-words-page .material-head{display:none}.match-words-page .match-search{z-index:30;background:#f7f6f2;padding:12px 0 10px;margin-top:-4px}.match-words-page .match-search-box input{padding:13px 44px 13px 42px}.match-clear{position:absolute;right:6px;top:50%;transform:translateY(-50%);display:none;width:32px;height:32px;padding:0;border:0;border-radius:50%;background:#f3f1ea;color:#22201c}.match-clear.show{display:block}.match-words-page .match-stats{margin:14px 0 10px}.match-words-page .match-toolbar{margin:12px 0 10px}.match-words-page .match-filters{margin-bottom:6px}.match-proto-item{border-top:1px solid #f0ede3}.match-proto-item-head{display:flex;align-items:center;gap:10px;padding:11px 14px;min-height:52px;cursor:pointer}.match-proto-item-head:hover,.match-proto-item.open{background:#fafaf6}.match-proto-item-main{flex:1;min-width:0}.match-proto-item-name{font-size:14px;font-weight:600}.match-proto-item-sub{color:#9a9587;font-size:12px}.match-proto-item-sub .no-price{color:#c2691a}.match-alias-count{border-radius:999px;padding:3px 10px;font-size:11.5px;font-weight:700;white-space:nowrap;background:#eaf7f5;color:#0d7377}.match-alias-count.zero{background:#f3f1ea;color:#9a9587}.match-item-chevron{color:#9a9587;font-size:14px}.match-proto-item.open .match-item-chevron{transform:rotate(90deg)}.match-proto-item-body{display:none;padding:0 14px 14px}.match-proto-item.open .match-proto-item-body{display:block}.match-section-label{font-size:12px;color:#767267;font-weight:700;margin:4px 0 6px}.match-aliases{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px}.match-alias{display:inline-flex;align-items:center;gap:6px;background:#fff;border:1px solid #e7e3d8;border-radius:999px;padding:4px 6px 4px 12px;font-size:13px}.match-alias button{border:0;background:#f3f1ea;color:#b3413a;width:24px;height:24px;padding:0;border-radius:50%;font-weight:700}.match-empty-alias{color:#9a9587;font-size:12.5px;margin-bottom:10px}.match-add-alias{display:flex;gap:8px}.match-add-alias input{flex:1;min-width:0;border-radius:10px;padding:10px 12px}.match-add-alias button,.match-item-actions button{min-height:40px;border-radius:10px;padding:9px 14px;font-weight:600}.match-add-alias button,.match-save{background:#0d7377;color:#fff}.match-item-actions{display:flex;gap:8px;margin-top:12px;padding-top:12px;border-top:1px dashed #e7e3d8}.match-item-actions .spacer{flex:1}.match-edit-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.match-field label{display:block;font-size:12px;color:#767267;margin:0 0 4px;font-weight:600}.match-field input,.match-field select{width:100%;padding:10px 12px;border:1px solid #d8d3c4;border-radius:10px;background:#fff}.match-delete{background:#fff;color:#b3413a;border:1px solid #b3413a}.match-plain{background:#f3f1ea;color:#22201c}.match-confirm{background:#fbeae8;border-radius:10px;padding:10px 12px;margin-top:10px;font-size:13px}.match-confirm-actions{display:flex;gap:8px;margin-top:8px}.match-confirm-actions .match-delete-confirm{background:#b3413a;color:#fff}.match-words-page .match-add-form{margin-top:12px}.match-words-page .match-add-form .row button:last-child{background:#0d7377;color:#fff}@media(max-width:600px){.material-page.match-words-page{padding:12px 12px 80px}.match-edit-grid{grid-template-columns:1fr}.match-item-actions button{flex:1}.match-item-actions .spacer{display:none}}
</style><script>
(()=>{
  const state={groupBy:"supplier",filter:"__all__",openGroups:new Set(),openItem:null,editItem:null,confirmDelete:null};
  const initialSearch=document.getElementById("dictSearch"),initialFilters=document.getElementById("matchFilters"),initialMode=document.getElementById("matchGroupMode"),list=document.getElementById("menuGroups");
  if(!initialSearch||!initialFilters||!initialMode||!list)return;
  const search=initialSearch.cloneNode(true);initialSearch.replaceWith(search);
  const filters=initialFilters.cloneNode(false);initialFilters.replaceWith(filters);
  const mode=initialMode.cloneNode(true);initialMode.replaceWith(mode);
  const clear=document.createElement("button");clear.type="button";clear.className="match-clear";clear.textContent="✕";clear.setAttribute("aria-label","ล้างคำค้นหา");search.parentNode.appendChild(clear);
  const aliasesFor=name=>ALIAS_TABLE.map((alias,index)=>Object.assign({},alias,{index})).filter(alias=>normalize(alias.canonical)===normalize(name));
  const el=(tag,className,text)=>{const node=document.createElement(tag);if(className)node.className=className;if(text!==undefined)node.textContent=text;return node};
  const optionList=(values,selected)=>values.map(value=>'<option value="'+escapeAttr(value)+'"'+(value===selected?' selected':'')+'>'+escapeAttr(value)+'</option>').join('');
  function testResult(){
    const result=document.getElementById("matchTestResult"),raw=search.value.trim();
    if(!result)return;
    if(!raw){result.innerHTML='<p class="match-hint">พิมพ์ชื่อเพื่อค้นหา — ระบบจะบอกด้วยว่าคำนี้จะถูกส่งให้ซัพพลายเออร์ไหน</p>';return}
    const matches=findSupplierMatches(raw);
    if(!matches.length){result.innerHTML='<div class="match-test none"><strong>❓ ยังไม่รู้จักคำนี้</strong>ถ้ามาในออเดอร์จริง ระบบจะให้เลือกซัพพลายเออร์เอง</div>';return}
    if(matches.length>1){result.innerHTML='<div class="match-test warn"><strong>⚠️ ตรงกับหลายซัพพลายเออร์</strong>'+matches.map(match=>escapeAttr(match.supplier)+' ('+escapeAttr(match.canonicalName)+')').join(', ')+' — ระบบจะให้เลือกเอง</div>';return}
    const match=matches[0];
    if(match.missingSupplier){result.innerHTML='<div class="match-test warn"><strong>⚠️ แมทได้ แต่ยังไม่มีซัพพลายเออร์</strong>แมทเป็น “'+escapeAttr(match.canonicalName)+'” แต่ยังไม่ได้ผูกซัพ</div>';return}
    if(match.viaContainment){result.innerHTML='<div class="match-test warn"><strong>🤔 ระบบเดาว่าเป็น '+escapeAttr(match.canonicalName)+'</strong>ซัพ '+escapeAttr(match.supplier)+' — ออเดอร์จริงจะให้ยืนยันก่อน</div>';return}
    const how=match.viaAlias?'ผ่านคำแมท':match.viaFirstWord?'จากคำแรก':'ตรงเป๊ะ';result.innerHTML='<div class="match-test ok"><strong>✅ ส่งให้ '+escapeAttr(match.supplier)+' ทันที</strong>'+how+' → “'+escapeAttr(match.canonicalName)+'” ไม่ต้องยืนยัน</div>';
  }
  function updateAddOptions(){
    const supplier=document.getElementById("newItemSupplier"),category=document.getElementById("newItemCategory");
    if(supplier)supplier.innerHTML=optionList(allSuppliers(),supplier.value)+'<option value="__new__">+ ซัพพลายเออร์ใหม่</option>';
    if(category)category.innerHTML=optionList(allCategories(),category.value)+'<option value="__newcat__">+ หมวดใหม่</option>';
  }
  function field(label,control){const wrap=el("div","match-field"),name=el("label",null,label);wrap.append(name,control);return wrap}
  function itemCard(index,term){
    const item=DICT[index],aliases=aliasesFor(item.name),card=el("article","match-proto-item"+(state.openItem===item.name||state.editItem===item.name?" open":""));
    const head=el("div","match-proto-item-head"),main=el("div","match-proto-item-main"),name=el("div","match-proto-item-name",item.name),sub=el("div","match-proto-item-sub");
    sub.innerHTML=(item.price?'฿'+escapeAttr(item.price):'<span class="no-price">ยังไม่ใส่ราคา</span>')+' · '+escapeAttr(state.groupBy==="supplier"?(item.category||"อื่นๆ"):'ซัพ '+item.supplier);
    const count=el("span","match-alias-count"+(aliases.length?"":" zero"),"🔗 "+aliases.length),chevron=el("span","match-item-chevron","›");main.append(name,sub);head.append(main,count,chevron);card.append(head);
    head.addEventListener("click",()=>{state.openItem=state.openItem===item.name?null:item.name;state.editItem=null;state.confirmDelete=null;renderMenuGroups()});
    const body=el("div","match-proto-item-body");
    if(state.editItem===item.name){
      const grid=el("div","match-edit-grid"),nameInput=el("input"),priceInput=el("input"),supplier=el("select"),category=el("select");nameInput.value=item.name;priceInput.value=item.price||"";supplier.innerHTML=optionList(allSuppliers(),item.supplier);category.innerHTML=optionList(allCategories(),item.category||"อื่นๆ");grid.append(field("ชื่อวัตถุดิบ",nameInput),field("ราคา/หน่วย",priceInput),field("ซัพพลายเออร์",supplier),field("หมวด",category));body.append(grid);
      const actions=el("div","match-item-actions"),cancel=el("button","match-plain","ยกเลิก"),spacer=el("span","spacer"),save=el("button","match-save","บันทึก");cancel.type=save.type="button";cancel.addEventListener("click",()=>{state.editItem=null;renderMenuGroups()});save.addEventListener("click",()=>{const old=item.name;item.name=nameInput.value.trim()||old;item.price=priceInput.value.trim();item.supplier=supplier.value;item.category=category.value;if(item.name!==old)ALIAS_TABLE.forEach(alias=>{if(normalize(alias.canonical)===normalize(old))alias.canonical=item.name});saveDict(DICT);saveAliasTable(ALIAS_TABLE);state.editItem=null;state.openItem=item.name;state.openGroups.add(state.groupBy==="supplier"?item.supplier:item.category);renderMenuGroups();showToast("บันทึกแล้ว")});actions.append(cancel,spacer,save);body.append(actions);
    }else{
      body.append(el("div","match-section-label",'คำที่ระบบจะรู้จักเป็น "'+item.name+'"'));
      if(aliases.length){const pills=el("div","match-aliases");aliases.forEach(alias=>{const pill=el("span","match-alias",alias.typed),remove=el("button",null,"×");remove.type="button";remove.title="ลบคำนี้";remove.addEventListener("click",()=>{ALIAS_TABLE.splice(alias.index,1);saveAliasTable(ALIAS_TABLE);state.openItem=item.name;renderMenuGroups();showToast('ลบคำ "'+alias.typed+'" แล้ว')});pill.append(remove);pills.append(pill)});body.append(pills)}else body.append(el("div","match-empty-alias","ยังไม่มีคำแมท — เพิ่มคำที่พนักงานมักพิมพ์ผิด/เรียกต่าง"));
      const addAlias=el("div","match-add-alias"),input=el("input"),add=el("button",null,"เพิ่ม");input.type="text";input.placeholder="เพิ่มคำ เช่น "+item.name+"xx";add.type="button";const saveAlias=()=>{const typed=input.value.trim();if(!typed){showToast("พิมพ์คำเทียบเคียงก่อน");return}upsertAlias(typed,item.name);state.openItem=item.name;renderMenuGroups();showToast('เพิ่มแมทคำ "'+typed+'" แล้ว')};add.addEventListener("click",saveAlias);input.addEventListener("keydown",event=>{if(event.key==="Enter")saveAlias()});addAlias.append(input,add);body.append(addAlias);
      if(state.confirmDelete===item.name){const confirm=el("div","match-confirm");confirm.innerHTML='ลบ "<b>'+escapeAttr(item.name)+'</b>" ออกจากระบบ? คำแมท '+aliases.length+' คำจะไม่มีวัตถุดิบปลายทาง';const row=el("div","match-confirm-actions"),cancel=el("button","match-plain","ยกเลิก"),yes=el("button","match-delete-confirm","ยืนยันลบ");cancel.type=yes.type="button";cancel.addEventListener("click",()=>{state.confirmDelete=null;renderMenuGroups()});yes.addEventListener("click",()=>{DICT.splice(index,1);saveDict(DICT);state.confirmDelete=null;state.openItem=null;renderMenuGroups();showToast('ลบ "'+item.name+'" แล้ว')});row.append(cancel,yes);confirm.append(row);body.append(confirm)}else{const actions=el("div","match-item-actions"),edit=el("button","match-edit","✏️ แก้ไขข้อมูล"),spacer=el("span","spacer"),remove=el("button","match-delete","🗑️ ลบ");edit.type=remove.type="button";edit.addEventListener("click",()=>{state.editItem=item.name;state.openItem=item.name;renderMenuGroups()});remove.addEventListener("click",()=>{state.confirmDelete=item.name;state.openItem=item.name;renderMenuGroups()});actions.append(edit,spacer,remove);body.append(actions)}
    }
    card.append(body);return card;
  }
  function renderGroups(){
    const term=normalize(search.value),keys=state.groupBy==="supplier"?allSuppliers():allCategories().filter(category=>DICT.some(item=>(item.category||"อื่นๆ")===category));
    if(!keys.includes(state.filter)&&state.filter!=="__all__"&&state.filter!=="__noprice__")state.filter="__all__";
    filters.innerHTML="";[["__all__","ทั้งหมด"]].concat(keys.map(key=>[key,key]),[["__noprice__","⚠️ ยังไม่ใส่ราคา"]]).forEach(pair=>{const button=el("button","match-filter"+(state.filter===pair[0]?" on":""),pair[1]);button.type="button";button.dataset.filter=pair[0];filters.append(button)});
    list.innerHTML="";let found=false;
    keys.forEach(key=>{if(state.filter!=="__all__"&&state.filter!=="__noprice__"&&state.filter!==key)return;const items=DICT.map((item,index)=>({item,index})).filter(entry=>(state.groupBy==="supplier"?entry.item.supplier:(entry.item.category||"อื่นๆ"))===key).filter(entry=>state.filter!=="__noprice__"||!entry.item.price).filter(entry=>!term||(normalize(entry.item.name)+normalize(entry.item.supplier)+normalize(entry.item.category||"")+aliasesFor(entry.item.name).map(alias=>normalize(alias.typed)).join(" ")).includes(term)).sort((a,b)=>a.item.name.localeCompare(b.item.name,"th"));if(!items.length)return;found=true;const group=el("section","match-group"+(!term&&state.filter==="__all__"&&!state.openGroups.has(key)?" closed":""));const head=el("div","match-group-head"),avatar=el("div","match-group-avatar",key.slice(0,2)),main=el("div","match-group-main"),title=el("div","match-group-name",(state.groupBy==="supplier"?"ซัพ ":"")+key),meta=el("div","match-group-meta",items.length+" รายการ · "+items.reduce((sum,entry)=>sum+aliasesFor(entry.item.name).length,0)+" คำแมท"),chevron=el("span","match-group-chevron","▾"),body=el("div","match-group-body");main.append(title,meta);head.append(avatar,main,chevron);head.addEventListener("click",()=>{const closed=group.classList.toggle("closed");if(closed)state.openGroups.delete(key);else state.openGroups.add(key)});items.forEach(entry=>body.append(itemCard(entry.index,term)));group.append(head,body);list.append(group)});
    if(!found){const empty=el("div","match-empty","ไม่พบวัตถุดิบที่ตรงกับคำค้นหา");list.append(empty)}
  }
  window.renderMenuGroups=function(){const count=document.getElementById("dictCount"),items=document.getElementById("matchStatItems"),aliases=document.getElementById("matchStatAlias"),noPrice=document.getElementById("matchStatNoPrice");if(count)count.textContent=DICT.length;if(items)items.textContent=DICT.length;if(aliases)aliases.textContent=ALIAS_TABLE.length;if(noPrice)noPrice.textContent=DICT.filter(item=>!item.price).length;updateAddOptions();clear.classList.toggle("show",!!search.value);testResult();renderGroups()};
  search.addEventListener("input",renderMenuGroups);clear.addEventListener("click",()=>{search.value="";renderMenuGroups();search.focus()});filters.addEventListener("click",event=>{const button=event.target.closest("button[data-filter]");if(!button)return;state.filter=button.dataset.filter;renderMenuGroups()});mode.addEventListener("click",event=>{const button=event.target.closest("button[data-mode]");if(!button)return;state.groupBy=button.dataset.mode;state.filter="__all__";state.openGroups.clear();mode.querySelectorAll("button").forEach(item=>item.classList.toggle("on",item===button));renderMenuGroups()});renderMenuGroups();
})();
</script>`;
...
export default {
  async fetch(request, env, ctx) {
    ...
    // New: per-day orders, replacing the old client-only localStorage kopi_orders_v1 /
    // kopi_daily_snapshot_v1 keys. GET returns that date's ORDERS array (kopi_orders_YYYY-MM-DD
    // in KV), POST replaces it wholesale — same request shape as /api/state above.
    if (url.pathname === "/api/orders") {
      ...
      if (request.method === "POST") {
        ...
        await env.KOPI_KV.put(key, JSON.stringify(orders));
        return Response.json(orders);
      }
      return new Response("Method not allowed", { status: 405 });
    }
    ...
    if (url.pathname === "/" || url.pathname === "/index.html") {
      return new Response(responsivePage(HTML_PAGE), { headers: { "Content-Type": "text/html; charset=UTF-8" } });
    }
