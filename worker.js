...
(()=>{let picker=null;const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const rows=()=>{const out=[];ORDERS.filter(o=>o.status!=="dispatched").forEach(o=>(o.items||[]).forEach(i=>out.push({o,i})));return out};const suppliers=()=>allSuppliers().filter(Boolean);function card(r){const k=r.o.id+'|'+r.i.id,open=picker===k;return '<article class="supplier-card"><div class="supplier-card-top"><div><div class="supplier-card-name">'+esc(r.i.label)+'</div><div class="supplier-card-meta">'+esc(r.o.department||r.o.orderer||'ออเดอร์')+'</div></div><div class="supplier-card-meta">'+esc(r.i.qty||'?')+' '+esc(r.i.unit||'')+'</div></div><div class="supplier-card-actions"><button data-ui="move" data-o="'+esc(r.o.id)+'" data-i="'+esc(r.i.id)+'">ย้ายซัพ</button><button data-ui="qty" data-o="'+esc(r.o.id)+'" data-i="'+esc(r.i.id)+'">แก้จำนวน</button><button data-ui="label" data-o="'+esc(r.o.id)+'" data-i="'+esc(r.i.id)+'">แก้ชื่อ</button></div>'+(open?'<select data-ui-supplier data-o="'+esc(r.o.id)+'" data-i="'+esc(r.i.id)+'"><option value="">เลือกซัพพลายเออร์</option>'+suppliers().map(s=>'<option value="'+esc(s)+'"'+(s===r.i.supplier?' selected':'')+'>'+esc(s)+'</option>').join('')+'</select>':'')+'</article>'}function renderSupplierUI(){const all=rows(),un=all.filter(r=>!r.i.supplier),assigned=all.filter(r=>r.i.supplier),list=document.querySelector('#ordersList'),board=document.querySelector('#supplierBoard');if(!list||!board)return;list.innerHTML=un.length?un.map(card).join(''):'<div class="supplier-empty">ไม่มีรายการที่ต้องเลือกซัพพลายเออร์</div>';const groups={};assigned.forEach(r=>(groups[r.i.supplier]??=[]).push(r));const names=[...new Set([...suppliers(),...Object.keys(groups)])];board.innerHTML=names.map(n=>'<section class="supplier-column"><div class="supplier-column-head"><span>'+esc(n)+'</span><span class="supplier-column-count">'+(groups[n]||[]).length+' รายการ</span></div>'+((groups[n]||[]).map(card).join('')||'<div class="supplier-empty">ยังไม่มีรายการ</div>')+'</section>').join('');const badge=document.querySelector('#pendingCountBadge');if(badge){badge.textContent=un.length;badge.style.display='inline-block'}const title=document.querySelector('#supplierSub');if(title)title.textContent=un.length?un.length+' รายการยังไม่ระบุซัพพลายเออร์':'จัดกลุ่มครบทุกรายการแล้ว'}const legacyDispatch=renderDispatch;renderOrders=renderSupplierUI;renderDispatch=function(){legacyDispatch()};document.addEventListener('click',e=>{const b=e.target.closest('[data-ui]');if(!b)return;const o=b.dataset.o,i=b.dataset.i;if(b.dataset.ui==='move'){picker=picker===o+'|'+i?null:o+'|'+i;renderOrders()}if(b.dataset.ui==='qty'){const r=rows().find(x=>x.o.id===o&&x.i.id===i),v=r&&prompt('จำนวนใหม่สำหรับ '+r.i.label,r.i.qty||'');if(v!==null&&r){editItemQty(o,i,v,r.i.unit);renderOrders();renderDispatch()}}if(b.dataset.ui==='label'){const r=rows().find(x=>x.o.id===o&&x.i.id===i),v=r&&prompt('ชื่อวัตถุดิบ',r.i.label);if(v&&r){editItemLabel(o,i,v.trim());saveOrders();renderOrders()}}});document.addEventListener('change',e=>{if(!e.target.matches('[data-ui-supplier]')||!e.target.value)return;picker=null;confirmItemSupplier(e.target.dataset.o,e.target.dataset.i,e.target.value)});renderOrders()})();
</script>`;
