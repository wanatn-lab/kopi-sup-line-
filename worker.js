...
const DIRECT_EDIT_TTL_SECONDS = 60 * 30; // a short, explicit edit session for the admin chat
const DIRECT_EDITOR_UNITS = ["", "ชิ้น", "ถุง", "ห่อ", "แพ็ค", "แพค", "กล่อง", "ขวด", "แกลลอน", "ลัง", "กิโลกรัม", "กก", "โล", "แผง", "แถว", "ใบ", "ฟอง", "หัว", "มัด", "คู่", "ฝา", "ขีด"];
...
function directEditSessionKey(userId){
  return "line_direct_edit:" + userId;
}

// LINE Flex cards cannot contain a keyboard numeric field. This parser powers a
// compact chat-based editor instead: one line per item, e.g. "ไข่เยี่ยวม้า = 3 แผง".
function parseDirectQuantityEdits(rawText, items){
  const applied = [];
  const errors = [];
  for (const rawLine of String(rawText || "").split("\n")){
    ...
    const item = (items || []).find((candidate) => lnNormalize(candidate.label) === lnNormalize(label));
    if (!item){
      errors.push(line);
      continue;
    }
    ...
    item.qty = (Number.isInteger(amount) ? amount : Math.round(amount * 100) / 100).toString();
    // Leave the old unit alone when it is omitted. Supplying a unit changes it,
    // so the admin can update both fields in the same compact message.
    const nextUnit = (match[3] || "").trim();
    if (nextUnit) item.unit = nextUnit.slice(0, 30);
    applied.push({ index: (items || []).indexOf(item), label: item.label, qty: item.qty, unit: item.unit || "" });
  }
  return { applied, errors };
}

function directEditorUnits(items){
  return Array.from(new Set(DIRECT_EDITOR_UNITS.concat((items || []).map((item) => item.unit || ""))));
}
...
function normalizeEditedQuantity(value){
  ...
  // Keep the orderer's original "unknown" markers.  They are common in LINE
  // orders (for example "ไข่ไก่ ="), and must not block moving another row.
  if (!text) return "?";
  if (["?", "=", "หมด", "เหลือ"].includes(text)) return text;
...
(() => {
  ...
  function render() {
    ...
    // Moving a supplier is an atomic action: save only this row immediately.
    // That way an unknown quantity or unfinished field in another row never
    // prevents the admin from moving the selected item.
    body.querySelectorAll(".supplier-select").forEach((select) => {
      select.addEventListener("change", async () => {
        const index = Number(select.dataset.supplierIdx);
        const item = items[index];
        const targetSupplier = select.value;
        if (!item || !targetSupplier || targetSupplier === item.supplier) return;
        const row = select.closest(".item-row");
        const qtyInput = row && row.querySelector("input[data-idx]");
        const unitSelect = row && row.querySelector(".unit-select");
        const unit = unitSelect && unitSelect.value !== "__custom_unit__" ? unitSelect.value : item.unit;
        select.disabled = true;
        try {
          await moveItemNow(index, targetSupplier, { qty: qtyInput ? qtyInput.value.trim() : item.qty, unit });
        } catch (error) {
          select.disabled = false;
          select.value = item.supplier || "";
          window.alert("ย้ายรายการไม่สำเร็จ: " + (error.message || error));
        }
      });
    });
    qs("#confirmBtn").disabled = false;
  ...
  async function moveItemNow(index, targetSupplier, changes) {
    const item = items[index];
    if (!item || !item.id) throw new Error("ไม่พบรหัสรายการ กรุณาปิดแล้วกดแก้ไขใหม่");
    const response = await fetch("/api/pending-order/item-move", {
      method: "POST",
      headers: authHeaders({ "Content-Type": "application/json" }),
      body: JSON.stringify({
        id: pendingId,
        itemId: item.id,
        sourceSupplier: unmatchedMode ? "" : supplierName,
    ...
    if (!response.ok) throw new Error(await response.text());
    items.splice(index, 1);
    qs("#subEl").textContent = "ย้าย " + item.label + " ไป " + targetSupplier + " แล้ว · เหลือ " + items.length + " รายการ";
    render();
  }
...
// The approved supplier and material-management mockups are translated here at response
// time.  Keeping the production page in one source string avoids a second, diverging UI
// implementation while retaining every existing parser and matching function below it.
const MODERN_INBOX = String.raw`<section class="main-tab-panel active" data-mainpanel="inbox">
  <div class="supplier-page">
    <div class="supplier-page-head">
      <div><h2>สั่งออเดอร์ซัพพลายเออร์</h2><p id="supplierPageSubtitle">จัดกลุ่มวัตถุดิบตามซัพพลายเออร์</p></div>
      <button class="btn-cta supplier-compose-btn" id="composeToggleBtn">+ วางข้อความสั่งซื้อใหม่</button>
    </div>
    <div class="compose-box supplier-compose" id="composeBox" style="display:none;">
      <div class="row2">
        <input type="text" id="ordererInput" placeholder="ชื่อผู้สั่ง (ไม่บังคับ)">
        <select id="departmentSelect"><option value="">แผนก/ที่มา (ไม่บังคับ)</option><option value="ร้านโกปี๊ หลังโรงไม้">ร้านโกปี๊ หลังโรงไม้</option><option value="The Old Offset">The Old Offset</option><option value="__new__">+ อื่นๆ (พิมพ์เอง)</option></select>
        <input type="text" id="departmentCustom" placeholder="พิมพ์ชื่อแผนก/ที่มา" style="display:none;">
      </div>
      <p class="muted">วางรายการจาก LINE ได้เลย ระบบจะคงการแยกร้านและการแมทคำเดิมไว้</p>
      <textarea id="rawInput" placeholder="วางข้อความสั่งซื้อจาก LINE ที่นี่..."></textarea>
      <div class="row supplier-compose-actions"><button class="secondary" id="composeCancelBtn">ยกเลิก</button><button class="btn-cta" id="processBtn">บันทึกออเดอร์ (0 รายการ)</button></div>
    </div>
    <div class="supplier-workspace">
      <aside class="supplier-inbox">
        <div class="supplier-section-title"><span>วัตถุดิบที่ต้องตรวจสอบ</span><span class="supplier-count" id="pendingCountBadge">0</span></div>
        <div class="supplier-stack" id="ordersList"></div>
      </aside>
      <main class="supplier-board-wrap">
        <div class="supplier-section-title"><span>จัดกลุ่มตามซัพพลายเออร์</span><span class="muted">ย้ายซัพหรือแก้จำนวนได้ทันที</span></div>
        <div class="supplier-board" id="supplierBoard"></div>
      </main>
    </div>
  </div>
</section>`;

const MODERN_DISPATCH = String.raw`<section class="main-tab-panel" data-mainpanel="dispatch">
  <div class="dispatch-page-head"><div><h2>สรุปออเดอร์แยกซัพพลายเออร์</h2><p>ตรวจสอบและส่งรายการที่จัดกลุ่มแล้ว</p></div><div class="row dispatch-actions"><button class="small secondary" id="viewYesterdayBtn" style="display:none;">🕐 ดูข้อมูลเมื่อวาน</button><button class="small secondary" id="dailyResetBtn">🗑️ ล้างหมด</button></div></div>
  <div id="results"><p class="muted">กำลังโหลดรายการ…</p></div>
</section>`;

const MODERN_MENU = String.raw`<section class="main-tab-panel" data-mainpanel="menu">
  <div class="material-page">
    <div class="material-head"><div><h2>จัดการวัตถุดิบ &amp; ราคา</h2><p>ตั้งราคาต่อหน่วย หมวดหมู่ และคำเทียบเคียงสำหรับการคำนวณออเดอร์</p></div><span class="material-badge" id="dictCount">0</span></div>
    <div class="material-toolbar"><input type="text" id="dictSearch" placeholder="ค้นหาวัตถุดิบ..."><button id="addItemToggleBtn">+ เพิ่มวัตถุดิบใหม่</button></div>
    <div class="material-add" id="addItemForm" style="display:none;">
      <div class="material-add-title">เพิ่มวัตถุดิบใหม่</div><div class="material-add-grid"><input type="text" id="newItemName" placeholder="ชื่อวัตถุดิบ"><select id="newItemCategory"></select><select id="newItemSupplier"></select><input type="text" id="newSupplierCustom" placeholder="พิมพ์ชื่อซัพพลายเออร์ใหม่" style="display:none;"><input type="text" id="newItemPrice" placeholder="ราคาต่อหน่วย (ไม่บังคับ)"></div><div class="material-add-actions"><button class="small" id="addDictBtn">บันทึกวัตถุดิบ</button></div>
    </div>
    <div id="dictSimilarWarning" class="material-warning" style="display:none;"></div>
    <div class="material-match-check"><label for="nameCheckInput">ทดสอบการแมทคำ</label><input type="text" id="nameCheckInput" placeholder="พิมพ์ชื่อวัตถุดิบเพื่อทดสอบ"><div id="nameCheckResult"></div></div>
    <div class="material-table-head"><span>วัตถุดิบ</span><span>หมวดหมู่ / ซัพพลายเออร์</span><span>ราคาต่อหน่วย</span><span></span></div>
    <div id="menuGroups" class="material-list"></div>
  </div>
</section>`;

const MODERN_UI = String.raw`<style>
  .supplier-page,.material-page,.dispatch-page-head{max-width:1180px;margin:0 auto}.supplier-page-head,.dispatch-page-head,.material-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:18px}.supplier-page h2,.dispatch-page-head h2,.material-head h2{margin:0;color:#1C1B1A;font-size:22px}.supplier-page-head p,.dispatch-page-head p,.material-head p{margin:4px 0 0;color:#6B6862;font-size:13px}.supplier-compose-btn{width:auto;padding:11px 18px}.supplier-compose{margin:0 0 18px;box-shadow:none}.supplier-compose-actions{justify-content:flex-end}.supplier-workspace{display:grid;grid-template-columns:minmax(280px,360px) minmax(0,1fr);gap:20px;align-items:start}.supplier-inbox,.supplier-board-wrap{background:#fff;border:1px solid #E5E3DE;border-radius:14px;padding:16px}.supplier-section-title{display:flex;align-items:center;justify-content:space-between;gap:8px;color:#6B6862;font-size:12px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;margin-bottom:12px}.supplier-count,.material-badge{background:#EDEBE6;color:#6B6862;border-radius:999px;padding:3px 9px;font-size:12px;font-weight:700;letter-spacing:0;text-transform:none}.supplier-stack{display:flex;flex-direction:column;gap:10px}.supplier-item{border:1px solid #E5E3DE;background:#F7F6F3;border-radius:12px;padding:12px}.supplier-item-top{display:flex;justify-content:space-between;gap:10px}.supplier-item-name{font-weight:700;font-size:14px;min-width:0}.supplier-item-meta{margin-top:3px;color:#6B6862;font-size:12px}.supplier-item-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}.supplier-link-btn{border:0;background:none;color:#2B4C7E;text-decoration:underline;font-size:12px;font-weight:700;padding:0;cursor:pointer}.supplier-item select{width:100%;margin-top:9px;padding:8px;border:1px solid #D8D3C4;border-radius:8px;background:#fff;font:inherit}.supplier-board{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.supplier-column{border:1px solid #E5E3DE;border-radius:12px;padding:12px;min-height:130px;background:#fff}.supplier-column-head{display:flex;justify-content:space-between;align-items:center;gap:6px;margin-bottom:10px;font-weight:700;font-size:13px}.supplier-column-count{background:#F1F0EC;color:#6B6862;border-radius:999px;padding:2px 7px;font-size:11px;font-weight:600}.supplier-column-empty{color:#9B968C;font-size:12px;padding:8px 0}.supplier-group-item{border:1px solid #E5E3DE;border-radius:10px;padding:10px;margin-top:8px;background:#F7F6F3}.supplier-group-item .supplier-item-actions{margin-top:7px}.supplier-order-tag{display:block;color:#9B968C;font-size:11px;margin-top:3px}.dispatch-page-head{padding:2px 0 14px;border-bottom:1px solid #E5E3DE}.dispatch-actions{margin:0}.material-page{max-width:960px;background:#fff;border:1px solid #E5E3DE;border-radius:14px;overflow:hidden}.material-head{padding:22px 20px 0;margin-bottom:0}.material-badge{white-space:nowrap}.material-toolbar{display:flex;gap:10px;padding:16px 20px;flex-wrap:wrap}.material-toolbar input{flex:1 1 220px;min-width:0;padding:10px 12px;border:1px solid #E5E3DE;border-radius:8px;background:#F7F6F3;font:inherit}.material-toolbar button{background:#2B4C7E}.material-add{margin:0 20px 16px;padding:16px;border:1px solid #E5E3DE;border-radius:12px;background:#F7F6F3}.material-add-title{font-size:13px;font-weight:700;color:#6B6862;margin-bottom:10px}.material-add-grid{display:grid;grid-template-columns:2fr 1.25fr 1.5fr 1fr;gap:10px}.material-add-grid input,.material-add-grid select{min-width:0;width:100%;padding:9px 10px;border:1px solid #E5E3DE;border-radius:8px;background:#fff;font:inherit}.material-add-actions{display:flex;justify-content:flex-end;margin-top:10px}.material-warning{margin:0 20px 14px;padding:10px;border-radius:8px;background:#FFF8E6;border:1px solid #E0C468;font-size:12.5px}.material-match-check{margin:0 20px 16px;padding:12px;border:1px solid #E5E3DE;border-radius:10px;background:#F7F6F3}.material-match-check label{display:block;font-size:12px;font-weight:700;color:#6B6862;margin-bottom:5px}.material-match-check input{width:100%;padding:9px 10px;border:1px solid #E5E3DE;border-radius:8px;font:inherit}.material-match-check #nameCheckResult{font-size:12px;margin-top:7px}.material-table-head{display:grid;grid-template-columns:minmax(180px,1fr) 200px 180px 72px;gap:14px;padding:0 20px 8px;color:#6B6862;font-size:11px;font-weight:700;letter-spacing:.04em;text-transform:uppercase}.material-list{padding:0 20px 20px}.category-group{margin:0}.category-group-head{display:none}.menu-item-card.material-row{display:grid;grid-template-columns:minmax(180px,1fr) 200px 180px 72px;gap:14px;align-items:center;padding:13px 0;border:0;border-top:1px solid #E5E3DE;border-radius:0;margin:0;background:#fff}.material-name{font-weight:700;font-size:14px}.material-aliases{margin-top:5px;color:#6B6862;font-size:11px}.material-tag{display:inline-flex;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;background:#EDEBE6;color:#5B5748;border-radius:999px;padding:4px 8px;font-size:11px;font-weight:700}.material-price-edit{display:flex;align-items:center;gap:5px}.material-price-edit input{width:88px;padding:8px;border:1px solid #E5E3DE;border-radius:8px;text-align:right;font:inherit}.material-price-edit span{font-size:12px;color:#6B6862}.material-row-actions{display:flex;gap:4px;justify-content:flex-end}.material-row-actions button{padding:5px;background:none;color:#2B4C7E;border:0;font-size:15px}.material-inline-form{grid-column:1/-1;display:none;gap:8px;grid-template-columns:2fr 1fr 1.2fr 1fr auto;padding:10px;background:#F7F6F3;border-radius:8px}.material-inline-form input,.material-inline-form select{min-width:0;padding:8px;border:1px solid #E5E3DE;border-radius:7px;font:inherit}.material-alias-form{grid-column:1/-1;display:none;gap:8px;grid-template-columns:1fr auto}.material-alias-form input{padding:8px;border:1px solid #E5E3DE;border-radius:7px;font:inherit}.material-empty{padding:24px;text-align:center;color:#9B968C;border:1px solid #E5E3DE;border-radius:12px}
  @media (max-width:700px){.supplier-page-head,.dispatch-page-head,.material-head{gap:10px}.supplier-page-head{flex-direction:column}.supplier-compose-btn{width:100%}.supplier-workspace{grid-template-columns:1fr;gap:12px}.supplier-inbox,.supplier-board-wrap{padding:14px}.supplier-board{grid-template-columns:1fr}.supplier-column{min-height:0}.dispatch-page-head{flex-direction:column}.dispatch-actions{width:100%;justify-content:space-between}.material-page{border-radius:12px}.material-head{padding:18px 14px 0}.material-head h2,.supplier-page h2,.dispatch-page-head h2{font-size:19px}.material-toolbar{padding:14px}.material-toolbar button{width:100%}.material-add{margin:0 14px 14px}.material-add-grid{grid-template-columns:1fr}.material-table-head{display:none}.material-list{padding:0 14px 14px}.menu-item-card.material-row{grid-template-columns:1fr auto;gap:9px;padding:13px 0}.material-row .material-meta{grid-column:1/-1}.material-price-edit{grid-column:1/2}.material-row-actions{grid-column:2/3;grid-row:1/2}.material-inline-form{grid-template-columns:1fr}.material-alias-form{grid-template-columns:1fr}.supplier-item-actions{gap:10px}}
</style><script>
(function(){
  var pickerFor = null;
  function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];});}
  function itemRows(){var rows=[];ORDERS.filter(function(o){return o.status!=="dispatched";}).forEach(function(o){(o.items||[]).forEach(function(i){rows.push({order:o,item:i});});});return rows;}
  function supplierList(){return allSuppliers().filter(Boolean);}
  function itemMarkup(row, grouped){var o=row.order,i=row.item,key=o.id+'|'+i.id,open=pickerFor===key;var actions='<button class="supplier-link-btn" data-modern-action="move" data-order="'+esc(o.id)+'" data-item="'+esc(i.id)+'">ย้ายซัพ</button><button class="supplier-link-btn" data-modern-action="qty" data-order="'+esc(o.id)+'" data-item="'+esc(i.id)+'">แก้จำนวน</button><button class="supplier-link-btn" data-modern-action="label" data-order="'+esc(o.id)+'" data-item="'+esc(i.id)+'">แก้ชื่อ</button>';var picker='';if(open){picker='<select data-modern-supplier data-order="'+esc(o.id)+'" data-item="'+esc(i.id)+'"><option value="">เลือกซัพพลายเออร์</option>'+supplierList().map(function(s){return '<option value="'+esc(s)+'"'+(s===i.supplier?' selected':'')+'>'+esc(s)+'</option>';}).join('')+'</select>';}return '<article class="'+(grouped?'supplier-group-item':'supplier-item')+'"><div class="supplier-item-top"><div><div class="supplier-item-name">'+esc(i.label)+'</div><span class="supplier-order-tag">'+esc(o.department||o.orderer||'ออเดอร์')+'</span></div><div class="supplier-item-meta">'+esc(i.qty||'?')+' '+esc(i.unit||'')+'</div></div><div class="supplier-item-actions">'+actions+'</div>'+picker+'</article>';}
  function renderModernOrders(){var rows=itemRows(),unassigned=rows.filter(function(r){return !r.item.supplier;}),assigned=rows.filter(function(r){return r.item.supplier;}),list=document.getElementById('ordersList'),board=document.getElementById('supplierBoard');if(!list||!board)return;list.innerHTML=unassigned.length?unassigned.map(function(r){return itemMarkup(r,false);}).join(''):'<div class="material-empty">ไม่มีรายการที่ต้องเลือกซัพพลายเออร์</div>';var by={};assigned.forEach(function(r){(by[r.item.supplier]||(by[r.item.supplier]=[])).push(r);});var names=supplierList();Object.keys(by).forEach(function(s){if(names.indexOf(s)<0)names.push(s);});board.innerHTML=names.length?names.map(function(s){var rs=by[s]||[];return '<section class="supplier-column"><div class="supplier-column-head"><span>'+esc(s)+'</span><span class="supplier-column-count">'+rs.length+' รายการ</span></div>'+(rs.length?rs.map(function(r){return itemMarkup(r,true);}).join(''):'<div class="supplier-column-empty">ยังไม่มีรายการในกลุ่มนี้</div>')+'</section>';}).join(''):'<div class="material-empty">เพิ่มซัพพลายเออร์จากการย้ายรายการก่อน</div>';var sub=document.getElementById('supplierPageSubtitle');if(sub)sub.textContent=unassigned.length?unassigned.length+' รายการยังไม่ระบุซัพพลายเออร์ จากทั้งหมด '+rows.length+' รายการ':'จัดกลุ่มครบทุกรายการแล้ว';var pending=document.getElementById('pendingCountBadge');if(pending)pending.textContent=unassigned.length;var badge=document.getElementById('inboxBadge');if(badge){badge.textContent=unassigned.length;badge.style.display=unassigned.length?'inline-block':'none';}}
  var legacyRenderDispatch=renderDispatch;
  renderOrders=renderModernOrders;
  renderDispatch=function(){legacyRenderDispatch();var result=document.getElementById('results');if(result)result.classList.add('dispatch-results');};
  buildMenuItemCard=function(idx){var d=DICT[idx],card=document.createElement('div'),aliases=ALIAS_TABLE.filter(function(a){return normalize(a.canonical)===normalize(d.name);});card.className='menu-item-card material-row';card.dataset.name=normalize(d.name)+' '+normalize(d.supplier)+' '+normalize(d.category||'')+' '+aliases.map(function(a){return normalize(a.typed);}).join(' ');card.innerHTML='<div><div class="material-name">'+esc(d.name)+'</div><div class="material-aliases">'+(aliases.length?'แมทคำ: '+aliases.map(function(a){return esc(a.typed);}).join(', '):'ยังไม่มีคำเทียบเคียง')+'</div></div><div class="material-meta"><span class="material-tag">'+esc(d.category||'อื่นๆ')+' · '+esc(d.supplier||'ยังไม่ระบุ')+'</span></div><label class="material-price-edit">฿ <input type="number" min="0" step="0.01" value="'+esc(d.price||'')+'" placeholder="ยังไม่ระบุ" data-modern-price="'+idx+'"><span>/ หน่วย</span></label><div class="material-row-actions"><button title="เพิ่มแมทคำ" data-modern-action="alias" data-dict="'+idx+'">🔗</button><button title="แก้ไข" data-modern-action="edit-dict" data-dict="'+idx+'">✏️</button><button title="ลบ" data-modern-action="delete-dict" data-dict="'+idx+'">🗑️</button></div><div class="material-alias-form"><input placeholder="คำที่พนักงานพิมพ์/เรียกอีกแบบ"><button class="small" data-modern-action="save-alias" data-dict="'+idx+'">เพิ่ม</button></div><div class="material-inline-form"><input class="modern-name" value="'+esc(d.name)+'"><select class="modern-category">'+allCategories().map(function(c){return '<option'+(c===d.category?' selected':'')+'>'+esc(c)+'</option>';}).join('')+'</select><select class="modern-supplier">'+allSuppliers().map(function(s){return '<option'+(s===d.supplier?' selected':'')+'>'+esc(s)+'</option>';}).join('')+'</select><input class="modern-price" value="'+esc(d.price||'')+'" placeholder="ราคา"><button class="small" data-modern-action="save-dict" data-dict="'+idx+'">บันทึก</button></div>';return card;};
  renderMenuGroups=function(){var count=document.getElementById('dictCount'),container=document.getElementById('menuGroups');if(!container)return;if(count)count.textContent='ทั้งหมด '+DICT.length+' รายการ';container.innerHTML='';if(!DICT.length){container.innerHTML='<div class="material-empty">ยังไม่มีวัตถุดิบ</div>';}else{DICT.slice().sort(function(a,b){return a.name.localeCompare(b.name,'th');}).forEach(function(d){container.appendChild(buildMenuItemCard(DICT.indexOf(d)));});}var sup=document.getElementById('newItemSupplier'),cat=document.getElementById('newItemCategory');if(sup)sup.innerHTML=allSuppliers().map(function(s){return '<option value="'+esc(s)+'">'+esc(s)+'</option>';}).join('')+'<option value="__new__">+ ซัพพลายเออร์ใหม่</option>';if(cat)cat.innerHTML=allCategories().map(function(c){return '<option value="'+esc(c)+'">'+esc(c)+'</option>';}).join('')+'<option value="__newcat__">+ หมวดใหม่</option>';filterMenuRows();};
  document.addEventListener('click',function(e){var b=e.target.closest('[data-modern-action]');if(!b)return;var act=b.getAttribute('data-modern-action'),oid=b.getAttribute('data-order'),iid=b.getAttribute('data-item'),idx=Number(b.getAttribute('data-dict'));if(act==='move'){pickerFor=pickerFor===oid+'|'+iid?null:oid+'|'+iid;renderOrders();}else if(act==='qty'){var row=itemRows().filter(function(r){return r.order.id===oid&&r.item.id===iid;})[0];if(!row)return;var next=prompt('จำนวนใหม่สำหรับ '+row.item.label,row.item.qty||'');if(next!==null){editItemQty(oid,iid,next,row.item.unit);renderOrders();renderDispatch();}}else if(act==='label'){var row2=itemRows().filter(function(r){return r.order.id===oid&&r.item.id===iid;})[0];if(!row2)return;var name=prompt('ชื่อวัตถุดิบ',row2.item.label);if(name&&name.trim()){editItemLabel(oid,iid,name.trim());saveOrders();renderOrders();}}else if(act==='alias'){var c=b.closest('.material-row');var f=c.querySelector('.material-alias-form');f.style.display=f.style.display==='grid'?'none':'grid';}else if(act==='save-alias'){var c2=b.closest('.material-row'),typed=c2.querySelector('.material-alias-form input').value.trim();if(typed){upsertAlias(typed,DICT[idx].name);renderMenuGroups();}}else if(act==='edit-dict'){var c3=b.closest('.material-row'),form=c3.querySelector('.material-inline-form');form.style.display=form.style.display==='grid'?'none':'grid';}else if(act==='save-dict'){var c4=b.closest('.material-row'),f2=c4.querySelector('.material-inline-form'),d=DICT[idx],name2=f2.querySelector('.modern-name').value.trim()||d.name,category=f2.querySelector('.modern-category').value,supplier=f2.querySelector('.modern-supplier').value,price=f2.querySelector('.modern-price').value.trim();if(name2===d.name){upsertDict(name2,supplier,price,category);}else{d.name=name2;d.supplier=supplier;d.price=price;d.category=category;saveDict(DICT);}renderMenuGroups();}else if(act==='delete-dict'){if(confirm('ลบวัตถุดิบนี้ออกจากรายการ?')){DICT.splice(idx,1);saveDict(DICT);renderMenuGroups();}}});
  document.addEventListener('change',function(e){var el=e.target;if(el.matches('[data-modern-supplier]')){var supplier=el.value;if(supplier){pickerFor=null;confirmItemSupplier(el.getAttribute('data-order'),el.getAttribute('data-item'),supplier);}}if(el.matches('[data-modern-price]')){var i=Number(el.getAttribute('data-modern-price')),d=DICT[i];if(d){upsertDict(d.name,d.supplier,el.value,d.category);renderMenuGroups();}}});
  renderOrders();renderMenuGroups();
})();
</script>`;

function modernizeHtml(html) {
  const withoutOrderStorage = html
    .replace(/function loadOrders\(\)\{[\s\S]*?\n\}\nfunction saveOrders\(\)\{[\s\S]*?\n\}/, 'function loadOrders(){ return []; }\nfunction saveOrders(){ postOrders(); }')
    .replace('const cloudOrders = await fetchOrders(todayDateStr());\n  ORDERS = cloudOrders !== null ? cloudOrders : loadOrders();\n  saveOrders();', 'const cloudOrders = await fetchOrders(todayDateStr());\n  ORDERS = cloudOrders !== null ? cloudOrders : [];\n  if (cloudOrders === null) setCloudStatus("⚠️ โหลดออเดอร์จากคลาวด์ไม่สำเร็จ — ลองรีเฟรชอีกครั้ง");')
    .replace(/<section class="main-tab-panel active" data-mainpanel="inbox">[\s\S]*?<\/section>/, MODERN_INBOX)
    .replace(/<section class="main-tab-panel" data-mainpanel="dispatch">[\s\S]*?<\/section>/, MODERN_DISPATCH)
    .replace(/<section class="main-tab-panel" data-mainpanel="menu">[\s\S]*?<\/section>/, MODERN_MENU);
  return withoutOrderStorage.replace('</body>', MODERN_UI + '</body>');
}
...
export default {
  async fetch(request, env, ctx) {
      ...
      ctx.waitUntil((async () => {
        ...
        try {
          if (event.type === "message" && event.message && event.message.type === "text") {
            ...
            // A compact, text-based numeric editor. It avoids a very tall Flex
            // card while letting the admin paste several exact quantities at once.
            const sessionKey = senderId ? directEditSessionKey(senderId) : "";
            const editSession = sessionKey && isAdminLineUser(env, senderId, activeAdminId)
              ? await env.KOPI_KV.get(sessionKey, { type: "json" })
              : null;
            if (editSession && editSession.pendingId && editSession.supplierName) {
              const command = setupText;
              const record = await env.KOPI_KV.get("line_pending:" + editSession.pendingId, { type: "json" });
              const items = record && record.bySupplier && record.bySupplier[editSession.supplierName];
              if (!Array.isArray(items)) {
                await env.KOPI_KV.delete(sessionKey);
                const expiredReply = await lineReply(
                  event.replyToken,
                  [{ type: "text", text: "ไม่พบรายการนี้แล้วครับ อาจถูกส่งหรือลบไปแล้ว" }],
                  env.LINE_CHANNEL_ACCESS_TOKEN
                );
                if (!expiredReply.ok) console.error("[line direct-edit expired reply failed]", expiredReply.status, await expiredReply.text());
                continue;
              }

              if (/^(จบ|เสร็จ|กลับ|ยกเลิก)$/i.test(command)) {
                await env.KOPI_KV.delete(sessionKey);
                const flexMessage = buildOpenOrderFlexMessage(record, editSession.pendingId, liffBaseUrl);
                const doneReply = await lineReply(event.replyToken, [flexMessage], env.LINE_CHANNEL_ACCESS_TOKEN);
                if (!doneReply.ok) console.error("[line direct-edit done reply failed]", doneReply.status, await doneReply.text());
                continue;
              }

              const result = parseDirectQuantityEdits(event.message.text, items);
              if (result.applied.length) {
                // A quantity of 0 intentionally removes the line from this supplier.
                record.bySupplier[editSession.supplierName] = items.filter((item) => item.qty !== "0");
                await env.KOPI_KV.put("line_pending:" + editSession.pendingId, JSON.stringify(record), { expirationTtl: PENDING_TTL_SECONDS });
              }
              const successLines = result.applied.map((item) => "• " + item.label + " = " + item.qty + (item.unit ? " " + item.unit : ""));
              const errorLine = result.errors.length
                ? "\n⚠️ ไม่พบชื่อหรือรูปแบบ: " + result.errors.slice(0, 2).join(" / ")
                : "";
              const replyText = result.applied.length
                ? "✅ แก้ไข " + editSession.supplierName + " แล้ว\n" + successLines.join("\n") + errorLine + "\n\nพิมพ์เพิ่มได้ หรือพิมพ์ “จบ” เพื่อกลับสรุป"
                : "พิมพ์ชื่อ จำนวน และหน่วยใหม่ได้เลย เช่น\nไข่เยี่ยวม้า = 3 แผง\nกระดาษทิชชู่ = 2 ห่อ\n(ไม่พิมพ์หน่วย = ใช้หน่วยเดิม)\n\nพิมพ์ “จบ” เพื่อกลับสรุป" + errorLine;
              const oneEditedItem = result.applied.length === 1 ? result.applied[0] : null;
              const unitQuickReply = oneEditedItem && oneEditedItem.qty !== "0"
                ? {
                    items: DIRECT_EDITOR_UNITS.slice(1, 13).map((unit) => ({
                      type: "action",
                      action: {
                        type: "postback",
                        label: unit,
                        data: "directunit:" + editSession.pendingId + ":" + encodeURIComponent(editSession.supplierName) + ":" + oneEditedItem.index + ":" + encodeURIComponent(unit),
                        displayText: "เปลี่ยนหน่วย " + oneEditedItem.label + " เป็น " + unit
                      }
                    }))
                  }
                : undefined;
              const editReply = await lineReply(
                event.replyToken,
                [{ type: "text", text: truncateLineText(replyText, 4500), ...(unitQuickReply ? { quickReply: unitQuickReply } : {}) }],
                env.LINE_CHANNEL_ACCESS_TOKEN
              );
              if (!editReply.ok) console.error("[line direct-edit reply failed]", editReply.status, await editReply.text());
              continue;
            }
            ...
            if (data.startsWith("splitqueue:")) {
            ...
            } else if (data.startsWith("directunit:")) {
              // Unit chips are a compact alternative to a long edit card.
              const parts = data.slice("directunit:".length).split(":");
              const pendingId = parts[0];
              const supplierName = decodeURIComponent(parts[1]);
              const itemIdx = parseInt(parts[2], 10);
              const unit = decodeURIComponent(parts[3] || "");
              const record = await env.KOPI_KV.get("line_pending:" + pendingId, { type: "json" });
              if (record && record.bySupplier && record.bySupplier[supplierName] && record.bySupplier[supplierName][itemIdx] && DIRECT_EDITOR_UNITS.includes(unit)) {
                record.bySupplier[supplierName][itemIdx].unit = unit;
                await env.KOPI_KV.put("line_pending:" + pendingId, JSON.stringify(record), { expirationTtl: PENDING_TTL_SECONDS });
                const item = record.bySupplier[supplierName][itemIdx];
                const res = await lineReply(
                  event.replyToken,
                  [{ type: "text", text: "✅ เปลี่ยนหน่วย " + item.label + " เป็น " + unit + " แล้ว\nพิมพ์จำนวน/หน่วยรายการถัดไปได้ หรือพิมพ์ “จบ” เพื่อกลับสรุป" }],
                  env.LINE_CHANNEL_ACCESS_TOKEN
                );
                if (!res.ok) console.error("[line directunit reply failed]", res.status, await res.text());
              } else {
                const res = await lineReply(event.replyToken, [{ type: "text", text: "ไม่พบรายการนี้แล้ว กรุณากดแก้ไขอีกครั้งครับ" }], env.LINE_CHANNEL_ACCESS_TOKEN);
                if (!res.ok) console.error("[line directunit missing reply failed]", res.status, await res.text());
              }

            } else if (data.startsWith("editsup:")) {
              // Start a short text editor instead of rendering every item with
              // +/- buttons. It is far easier to use for a long supplier order.
              ...
              if (postbackUserId) {
                await env.KOPI_KV.put(
                  directEditSessionKey(postbackUserId),
                  JSON.stringify({ pendingId, supplierName }),
                  { expirationTtl: DIRECT_EDIT_TTL_SECONDS }
                );
              }
              const examples = items.slice(0, 2).map((item) => item.label + " = " + (item.qty || "1") + (item.unit ? " " + item.unit : ""));
              const prompt = items.length
                ? "✏️ แก้ไขจำนวน/หน่วย: " + supplierName + "\n\nพิมพ์ชื่อ จำนวน และหน่วยใหม่ได้หลายบรรทัด เช่น\n" + examples.join("\n") + "\n\nไม่พิมพ์หน่วย = ใช้หน่วยเดิม · พิมพ์ “จบ” เพื่อกลับสรุป · ใส่ 0 เพื่อลบรายการ"
                : "ไม่พบรายการสำหรับ " + supplierName + " แล้วครับ";
              const res = await lineReply(
                event.replyToken,
    ...
    // ---------------- Move one LIFF row immediately ----------------
    if (url.pathname === "/api/pending-order/item-move") {
      const owner = await getVerifiedLiffOwnerId(request, env);
      if (owner.error) return owner.error;
      if (request.method !== "POST") return new Response("Method not allowed", { status: 405 });

      let body;
      try { body = await request.json(); } catch (error) { return new Response("Bad JSON", { status: 400 }); }
      const { id, itemId, sourceSupplier, targetSupplier, unmatched: unmatchedMode } = body || {};
      if (!id || typeof itemId !== "string" || !itemId || typeof targetSupplier !== "string" || !targetSupplier) {
      ...
      const sourceItems = unmatchedMode
        ? record.unmatched
        : (typeof sourceSupplier === "string" ? record.bySupplier[sourceSupplier] : null);
      if (!Array.isArray(sourceItems)) return new Response("Source supplier not found", { status: 404 });
      const index = sourceItems.findIndex((item) => item && item.id === itemId);
      if (index < 0) return new Response("Item not found", { status: 404 });

      const item = sourceItems[index];
      const qty = normalizeEditedQuantity(body.qty === undefined ? item.qty : body.qty);
      const unit = normalizeEditedUnit(body.unit === undefined ? item.unit : body.unit, item.unit);
      if (qty === null || unit === null) return new Response("Invalid item, quantity, or unit", { status: 400 });

      sourceItems.splice(index, 1);
      item.qty = qty;
      item.unit = unit;
      if (!record.bySupplier[targetSupplier]) record.bySupplier[targetSupplier] = [];
      record.bySupplier[targetSupplier].push(item);
      if (!unmatchedMode && sourceSupplier !== targetSupplier && !sourceItems.length) delete record.bySupplier[sourceSupplier];

      const existingOrder = Array.isArray(record.supplierOrder) ? record.supplierOrder : Object.keys(record.bySupplier);
      record.supplierOrder = existingOrder.filter((supplier) => Array.isArray(record.bySupplier[supplier]) && record.bySupplier[supplier].length);
      if (!record.supplierOrder.includes(targetSupplier)) record.supplierOrder.push(targetSupplier);
      if (record.sentSuppliers && record.sentSuppliers[targetSupplier]) delete record.sentSuppliers[targetSupplier];
      record.updatedAt = Date.now();
      await env.KOPI_KV.put("line_pending:" + id, JSON.stringify(record), { expirationTtl: PENDING_TTL_SECONDS });
      await pushLatestDraftCard(env, owner.userId, id, record, "ย้าย " + (item.label || "รายการ") + " ไป " + targetSupplier + " แล้ว");
      return Response.json({ item, supplier: targetSupplier });
    }
    ...
    if (url.pathname === "/api/pending-order") {
      ...
      if (request.method === "GET") {
        ...
        if (!record || !record.bySupplier) return new Response("Not found", { status: 404 });
        ensurePendingItemIds(record);
        // Persist generated IDs before returning them to LIFF.  Otherwise a
        // follow-up one-row move would not be able to find the same item.
        await env.KOPI_KV.put("line_pending:" + id, JSON.stringify(record), { expirationTtl: PENDING_TTL_SECONDS });
        const state = (await env.KOPI_KV.get(STATE_KEY, { type: "json" })) || {};
        const suppliers = getKnownSuppliers(Array.isArray(state.dict) ? state.dict : []);
        if (unmatchedMode) {
          return Response.json({ suppliers, items: record.unmatched });
        }
    ...
    if (url.pathname === "/api/orders") {
      ...
      const dayKey = "kopi_orders_" + date;

      if (request.method === "GET") {
        const day = await env.KOPI_KV.get(dayKey, { type: "json" });
        return Response.json(day || { date, orders: [] });
      }
      ...
      if (request.method === "POST") {
        ...
        const dayData = { date, orders, updatedAt: Date.now() };
        await env.KOPI_KV.put(dayKey, JSON.stringify(dayData));

        // Track which days exist + delete anything older than 7 days (today + 6 back).
        const index = (await env.KOPI_KV.get("kopi_orders_index", { type: "json" })) || { dates: [] };
        ...
        for (const d of index.dates) {
          if (d < cutoffStr) {
            await env.KOPI_KV.delete("kopi_orders_" + d);
          } else {
            keepDates.push(d);
          }
        }
        index.dates = keepDates;
        await env.KOPI_KV.put("kopi_orders_index", JSON.stringify(index));
    ...
    if (url.pathname === "/api/orders/history") {
      ...
      const index = (await env.KOPI_KV.get("kopi_orders_index", { type: "json" })) || { dates: [] };
      const sortedDates = index.dates.slice().sort().reverse();
      const days = await Promise.all(
        sortedDates.map((d) => env.KOPI_KV.get("kopi_orders_" + d, { type: "json" }))
      );
      return Response.json({ days: days.filter(Boolean) });
    }

    if (url.pathname === "/" || url.pathname === "/index.html") {
      return new Response(modernizeHtml(HTML_PAGE), { headers: { "Content-Type": "text/html; charset=UTF-8" } });
    }
